...
import { StartingOrder, Discipline, AthleteID, IAthleteID} from "@/app/lib/interfaces";
...
export function DisciplineEditButton(
  ...
  const showResults = function() {
    setState({ ...state, showPopupLoading: true })
    // TODO: Get athelte results
    get_group_achievements(group_name, (fetched_athletes: Athlete[]) => {
        ...
        fetched_athletes.forEach((athlete: Athlete) => {
            Object.values(athlete.achievements).forEach((achievement: AchievementValue) => {
                achievement.athlete_name = athlete.name + "_" + athlete.surname;
                let achievement_name = achievement.Time?.name || achievement.Distance?.name || achievement.Height?.name || "";
                if (achievement && achievement_name == state.discipline.name) {
                    athletes.push({
                        name: athlete.name,
          ...
          <div className="text-lg sm:text-xl text-center">
            ...
            {state.discipline.state == "Finished" &&
              <div
                ...
                className="border rounded-md shadow-black shadow-md p-2 m-4 bg-green-300 active:shadow-none"
              >
                {(state.showPopupLoading) ?
                  <div className="flex justify-center items-center w-full">
                    <LoadingAnimation width="h-8" height="w-8"></LoadingAnimation>
                  </div>
                  :
                  <div>Ergebnisse anzeigen!</div>
                }
              </div>
            }
            {state.discipline.state == "Finished" &&
